...
if(empty($news_data_array)){
  ...
  // エラー回避処理、現在のページが総ページよりも大きい場合は（get値で適当な値がきたら）
	...
	$page_html = get_posts_pagenation($sou_page=$all_page, $gen_page=$current_page, $sento="<i class='fa-arrow-left'></i>", $saikoubi="<i class='fa-arrow-right'></i>", $page_option="cocorotosou_tag/".$term."/?");
}

$myterm = get_term_by('slug', $term, 'cocorotosou_tag');
$myterm_name = "すべての記事";
    		...
    		<div class="sec_inner maxWidth">
    			...
    			$args = array(
    				...
    				'tax_query' => array(
    					'relation' => 'AND',
    					array(
    						'taxonomy' => 'cocorotosou_tag', /* 指定したい投稿タイプが持つタクソノミーを指定 */
    						'field' => 'slug',
    			...
    			<div class="page_link">
    				<div class="pagination textCenterCm">
    					<?php if(count($news_data_array) >= 11){echo $page_html;}//数字は件数+1 ?>
    				</div>
    			</div>
    		</div><!--end sec_inner-->
